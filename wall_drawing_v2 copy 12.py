...
class EnhancedWallDetector:
    ...
    def calculate_parameters(self, sensitivity: float) -> dict:
        ...
        max_params = {
            'threshold_value': 170,    # Reduced from 180 for better thin line detection
            'canny_low': 35,          # Reduced from 30
            'canny_high': 120,         # Reduced from 120
            'hough_threshold': 50,     # Reduced from 50
            'min_line_length': 35,     # Reduced from 60
            'max_line_gap': 35,        # Increased from 25
            'angle_tolerance': 3.0,
            'duplicate_tolerance': 11,   # Reduced from 12
            'min_wall_thickness': 2,    # Reduced from 3
            'outer_wall_threshold': 0.75,
            'junction_tolerance': 15,
            'min_room_area': 500,
            'wall_darkness_threshold': 180  # Increased from 150
        }
